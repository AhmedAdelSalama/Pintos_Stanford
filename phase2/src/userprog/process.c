...
start_process (void *file_name_)
{
  ...
  if (!success){
    ...
    if(parent_thread!=NULL){
      ...
      list_push_front(&parent_thread->children, &thread_current()->child);
      sema_up(&parent_thread->wait_child);
      sema_down(&thread_current()->wait_child);
    }
  }
...
process_wait (tid_t child_tid) 
{
  ...
  if(child_thread==NULL)
    return -1;
    /* isn't it thread_currnent(), too? */
  child_thread->parent->waiting_on_thread = child_tid;
  /* remove child from children list */
  list_remove(&child_thread->child);
  sema_up(&child_thread->wait_child);
  sema_down(&child_thread->parent->wait_child);


  /* Later return child_tid which was stored earlier in parent->waiting_on_thread */
  return child_thread->parent->waiting_on_thread;
}
