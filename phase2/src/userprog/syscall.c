...
exit(int status )
{
  /* Release all resources 
      - close all files
      - make sure there is no resource leakage*/
  for(struct list_elem *e = list_begin(&thread_current()->files);
                        e!=list_end(&thread_current()->files);
                        e = list_next(e)){
    struct open_file *temp = list_entry(e, struct open_file, file_elem);
      acquire_files_synch_lock();
  ...
  if(thread_current()->parent->waiting_on_thread == thread_current()->tid){
    ...
    thread_current()->parent->waiting_on_thread = -1;
